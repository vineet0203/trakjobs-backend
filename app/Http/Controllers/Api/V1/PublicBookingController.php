...
class PublicBookingController extends BaseController
{
    ...
    public function store(Request $request): JsonResponse
    {
        $validated = $request->validate([
            ...
            'vendor_ids' => ['nullable', 'array', 'max:5'],
        ...
        try {
            ...
            $matchingVendors = collect();
            if (!empty($vendorIds)) {
                $matchingVendors = \App\Models\Vendor::whereIn('id', $vendorIds)
                    ->where('status', 'active')
                    ->get();
            }
            ...
            if ($matchingVendors->isEmpty()) {
                // Create unassigned quote when no vendors found or selected
                $quote = Quote::create([
                    'quote_number' => Quote::generateQuoteNumber(),
                    'title' => 'New Unassigned Lead: ' . str_replace('_', ' ', $validated['service_sub_category']),
                    'client_id' => null,
